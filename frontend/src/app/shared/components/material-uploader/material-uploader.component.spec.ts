 ...
 *  - FAIL-FAST bad MIME: non-whitelisted type → error toast, presign NOT called.
 *  - HAPPY FLOW: valid file → presign → uploadToStorage(XHR) → confirm → uploaded emitted.
...
const PRESIGN_RESPONSE = {
  ...
  key: 'courses/c-1/materials/uuid-slides.pdf',
...
describe('MaterialUploaderComponent', () => {
  ...
  it('HAPPY FLOW: valid PDF → presign called → uploadToStorage called → confirm called → uploaded emitted', async () => {
    const fixture = TestBed.createComponent(MaterialUploaderComponent);
    const comp = fixture.componentInstance;
    comp.courseId = 'c-1';
    ...
    // Step 1: presign called with correct body
    expect(materialServiceSpy.presign).toHaveBeenCalledWith('c-1', {
      nombre: 'slides.pdf',
    ...
    expect(materialServiceSpy.confirm).toHaveBeenCalledWith('c-1', {
  ...
  it('progress signal resets to 0 at start of upload and reaches 100 on success', async () => {
    ...
    // The important assertion is that the upload went to completion.
